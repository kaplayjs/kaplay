...
onLoad(() => {
    ...
    for (let i = 0; i < bean.width; i++) {
        objSlices.push(bean.frames[0].scale(new Quad(i / bean.width, 0, 1 / bean.width, 1)))
    }

    wall = getSprite("wall").data
    for (let i = 0; i < wall.width; i++) {
        slices.push(wall.frames[0].scale(new Quad(i / wall.width, 0, 1 / wall.width, 1)))
    }
})
...
function rayCastGrid(origin, direction, gridPosHit, maxDistance = 64) {
    ...
    let dist = vec2((step.x > 0) ? (gridPos.x + 1 - origin.x) : (origin.x - gridPos.x),
        (step.y > 0) ? (gridPos.y + 1 - origin.y) : (origin.y - gridPos.y));
    let tMax = vec2((tDelta.x < Infinity) ? tDelta.x * dist.x : Infinity,
        ...
        draw() {
            ...
            for (let x = 0; x <= 120; x++) {
                ...
                if (hit) {
                    ...
                    let u = Math.abs(hit.normal.x) > Math.abs(hit.normal.y) ? hit.point.y : hit.point.x
                    u = (u % 16) / 16
                    u = u - Math.floor(u)
                    // Height of the wall
