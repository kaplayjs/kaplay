...
function setGravityDirection(d) {
    ...
    shared_1._k.game.gravity = d.unit().scale(
        shared_1._k.game.gravity ? shared_1._k.game.gravity.len() : 1,
    );
}
function getGravityDirection() {
    // If gravity != null return unit vector, otherwise return (0, 1)
    return shared_1._k.game.gravity
        ? shared_1._k.game.gravity.unit()
        : (0, math_1.vec2)(0, 1);
}
