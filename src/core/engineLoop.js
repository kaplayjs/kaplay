    ...
    app.run(function () {
            ...
            if (assets.loaded) {
                if (!debug.paused) {
                    for (var _i = 0, _a = game
                        .systemsByEvent[systems_1.LCEvents.BeforeFixedUpdate]; _i < _a.length; _i++) {
                        var sys = _a[_i];
                        sys.run();
                    }
                    frameRenderer.fixedUpdateFrame();
                    for (var _b = 0, _c = game
                        .systemsByEvent[systems_1.LCEvents.AfterFixedUpdate]; _b < _c.length; _b++) {
                        var sys = _c[_b];
                        sys.run();
                    }
                }
        ...
        try {
            ...
            if (!assets.loaded) {
                if ((0, asset_1.loadProgress)() === 1 && !isFirstFrame) {
                    ...
                    (0, asset_1.getFailedAssets)().forEach(function (details) {
                        var _a;
                        return (_a = game.events).trigger.apply(_a, __spreadArray(["loadError"], details, false));
                    });
            ...
            if (!assets.loaded && gopt.loadingScreen !== false
                ...
                if (!debug.paused) {
                    for (var _i = 0, _a = game
                        .systemsByEvent[systems_1.LCEvents.BeforeUpdate]; _i < _a.length; _i++) {
                        var sys = _a[_i];
                        sys.run();
                    }
                    frameRenderer.updateFrame();
                    for (var _b = 0, _c = game
                        .systemsByEvent[systems_1.LCEvents.AfterUpdate]; _b < _c.length; _b++) {
                        var sys = _c[_b];
                        sys.run();
                    }
                }
                // checkFrame();
                frameRenderer.frameStart();
                for (var _d = 0, _e = game.systemsByEvent[systems_1.LCEvents.BeforeDraw]; _d < _e.length; _d++) {
                    var sys = _e[_d];
                    sys.run();
                }
                ...
                for (var _f = 0, _g = game.systemsByEvent[systems_1.LCEvents.AfterDraw]; _f < _g.length; _f++) {
                    var sys = _g[_f];
                    sys.run();
                }
                frameRenderer.frameEnd();
