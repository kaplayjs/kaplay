...
import { type StrictWrapper, withFullCtx } from "./context";
...
export const ctxMake = withFullCtx(
    function make<T>(
        app: App,
        gfx: AppGfxCtx,
        game: any,
        ...
        const obj: Omit<GameObj, keyof typeof evs> = {
            ...
            add<T2>(this: GameObj, a: CompList<T2> | GameObj<T2>): GameObj {
                const obj = Array.isArray(a) ? k.current.make(a) : a;
                if (obj.parent) {
                    throw new Error(
                ...
                obj.trigger("add", obj);
                game.current.events.trigger("add", obj);
                return obj;
            },
            ...
            remove(obj: GameObj): void {
                ...
                if (idx !== -1) {
                    ...
                    const trigger = (o: GameObj) => {
                        o.trigger("destroy");
                        game.current.events.trigger("destroy", o);
            ...
            draw(
                ...
                k.current.pushTransform();
                k.current.pushTranslate(this.pos);
                k.current.pushScale(this.scale);
                k.current.pushRotate(this.angle);
                const children = this.children.sort((o1, o2) => {
                    const l1 = o1.layerIndex ?? game.current.defaultLayerIndex;
                    const l2 = o2.layerIndex ?? game.current.defaultLayerIndex;
                    return (l1 - l2) || (o1.z ?? 0) - (o2.z ?? 0);
                });
                // TODO: automatically don't draw if offscreen
                if (this.mask) {
                    const maskFunc = {
                        intersect: k.current.drawMasked,
                        subtract: k.current.drawSubtracted,
                    }[this.mask];
                    if (!maskFunc) {
            ...
            get(t: Tag | Tag[], opts: GetOpt = {}): GameObj[] {
                ...
                if (opts.liveUpdate) {
                    ...
                    events.push(k.current.onAdd((obj) => {
                        if (isChild(obj) && obj.is(t)) {
                            list.push(obj);
                        }
                    }));
                    events.push(k.current.onDestroy((obj) => {
                        if (isChild(obj) && obj.is(t)) {
            ...
            exists(this: GameObj): boolean {
                return game.current.root.isAncestorOf(this);
            },
            ...
            trigger(name: string, ...args: unknown[]): void {
                ...
                game.current.objEvents.trigger(name, this, ...args);
            },
