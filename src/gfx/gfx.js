...
var Texture = /** @class */ (function () {
    function Texture(ctx, w, h, opt) {
        ...
        var filter = {
            "linear": gl.LINEAR,
            "nearest": gl.NEAREST,
        }[(_b = (_a = opt.filter) !== null && _a !== void 0 ? _a : ctx.opts.texFilter) !== null && _b !== void 0 ? _b : "nearest"];
        var wrap = {
            "repeat": gl.REPEAT,
...
var BatchRenderer = /** @class */ (function () {
    function BatchRenderer(ctx, format, maxVertices, maxIndices) {
        ...
        this.ctx = ctx;
        this.stride = format.reduce(function (sum, f) { return sum + f.size; }, 0);
    ...
    BatchRenderer.prototype.push = function (primitive, vertices, indices, shader, tex, uniform, blend, width, height, fixed) {
        ...
        if (this.picture) {
            ...
            if (this.picture.commands.length) {
                var lastCommand = this.picture.commands[this.picture.commands.length - 1];
        ...
        if (primitive !== this.curPrimitive
            || tex !== this.curTex
    ...
    BatchRenderer.prototype.flush = function (width, height) {
        ...
        this.curShader.send({
            width: width,
            height: height,
            camera: this.curFixed ? math_1.IDENTITY_MATRIX : (0, camera_1.getCamTransform)(),
...
var Mesh = /** @class */ (function () {
    ...
    Mesh.prototype.draw = function (primitive, index, count) {
        ...
        this.ctx.pushElementArrayBuffer(this.glIBuf);
        this.ctx.setVertexFormat(this.vertexFormat);
        gl.drawElements(primitive !== null && primitive !== void 0 ? primitive : gl.TRIANGLES, index !== null && index !== void 0 ? index : this.count, gl.UNSIGNED_SHORT, count !== null && count !== void 0 ? count : 0);
        this.ctx.popArrayBuffer();
        this.ctx.popElementArrayBuffer();
    };
...
function genStack(setFunc) {
    ...
    var cur = function () { return stack[stack.length - 1]; };
    return [push, pop, cur];
}
function initGfx(gl, opts) {
    ...
    function destroy() {
        gc.forEach(function (action) { return action(); });
    ...
    function setVertexFormat(fmt) {
        if ((0, deepEq_1.deepEq)(fmt, curVertexFormat))
            return;
        curVertexFormat = fmt;
        var stride = fmt.reduce(function (sum, f) { return sum + f.size; }, 0);
    ...
    }), pushTexture2D = _a[0], popTexture2D = _a[1];
    var _b = genStack(function (b) {
        return gl.bindBuffer(gl.ARRAY_BUFFER, b);
    }), pushArrayBuffer = _b[0], popArrayBuffer = _b[1];
    var _c = genStack(function (b) { return gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, b); }), pushElementArrayBuffer = _c[0], popElementArrayBuffer = _c[1];
    var _d = genStack(function (b) {
        return gl.bindFramebuffer(gl.FRAMEBUFFER, b);
    }), pushFramebuffer = _d[0], popFramebuffer = _d[1];
    var _e = genStack(function (b) { return gl.bindRenderbuffer(gl.RENDERBUFFER, b); }), pushRenderbuffer = _e[0], popRenderbuffer = _e[1];
    var _f = genStack(function (stack) {
        if (!stack)
            return;
        var x = stack.x, y = stack.y, w = stack.w, h = stack.h;
        gl.viewport(x, y, w, h);
    }), pushViewport = _f[0], popViewport = _f[1];
    var _g = genStack(function (p) {
        return gl.useProgram(p);
    }), pushProgram = _g[0], popProgram = _g[1];
    pushViewport({
        x: 0,
        y: 0,
