...
var FontData = /** @class */ function() {
    function FontData(face, opt) {
        ...
        this.fontface = face;
        this.filter = (_a = opt.filter) !== null && _a !== void 0
            ? _a
            : general_1.DEF_FONT_FILTER;
        this.size = (_b = opt.size) !== null && _b !== void 0
            ? _b
            : general_1.DEF_TEXT_CACHE_SIZE;
        if (this.size > general_1.MAX_TEXT_CACHE_SIZE) {
            throw new Error(
                "Max font size: ".concat(general_1.MAX_TEXT_CACHE_SIZE),
            );
        }
        if (opt.outline) {
            this.outline = {
...
function resolveFont(src) {
    ...
    if (!src) {
        return resolveFont(
            (_a = shared_1._k.globalOpt.font) !== null && _a !== void 0
                ? _a
                : general_1.DEF_FONT,
        );
    }
    if (typeof src === "string") {
        ...
        if (bfont) {
            ...
            document.fonts.check(
                "".concat(general_1.DEF_TEXT_CACHE_SIZE, "px ").concat(src),
            )
        ) {
            return src;
        }
...
function getFont(name) {
    var _a;
    return (_a = shared_1._k.assets.fonts.get(name)) !== null && _a !== void 0
        ? _a
        : null;
}
...
function loadFont(name, src, opt) {
    ...
    var font = new FontFace(
        name,
        typeof src === "string" ? "url(".concat(src, ")") : src,
    );
    document.fonts.add(font);
    return shared_1._k.assets.fonts.add(
        name,
        font.load().catch(function(err) {
            throw new Error(
                "Failed to load font from \"".concat(src, "\": ").concat(err),
            );
        }).then(function(face) {
            return new FontData(face, opt);
        }),
    );
}
