...
function loadSpriteAtlas(src, data) {
    ...
    if (typeof data === "string") {
        return (0, asset_1.load)(new Promise(function (res, rej) {
            (0, asset_1.fetchJSON)(data).then(function (json) {
                loadSpriteAtlas(src, json).then(res).catch(rej);
            });
        }));
    }
    return (0, asset_1.load)(sprite_1.SpriteData.from(src).then(function (atlas) {
        var map = {};
        var _loop_1 = function (name_1) {
            ...
            var frames_1 = info.frames
                ? info.frames.map(function (f) {
                    return new math_1.Quad(quad.x + (info.x + f.x) / w * quad.w, quad.y + (info.y + f.y) / h * quad.h, f.w / w * quad.w, f.h / h * quad.h);
                })
                : (0, sprite_1.slice)(info.sliceX || 1, info.sliceY || 1, quad.x + info.x / w * quad.w, quad.y + info.y / h * quad.h, info.width / w * quad.w, info.height / h * quad.h);
            var spr = new sprite_1.SpriteData(atlas.tex, frames_1, info.anims);
