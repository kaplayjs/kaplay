...
function loadAseprite(name, imgSrc, jsonSrc) {
    ...
    return shared_1._k.assets.sprites.add(name, resolveJSON.then(function (data) {
        var size = data.meta.size;
        var frames = data.frames.map(function (f) {
            return new math_1.Quad(f.frame.x / size.w, f.frame.y / size.h, f.frame.w / size.w, f.frame.h / size.h);
        });
        var anims = {};
