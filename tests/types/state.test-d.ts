...
describe("Type Inference from state()", () => {
    ...
    test("StateComp.enterState() param should be typed", () => {
        expectTypeOf(stateComp.enterState).toEqualTypeOf<(state: "initial" | "final", ...args: any) => void>
    })
    ...
    test("StateComp.onEnterState args should be typed", () => {
        expectTypeOf(stateComp.onStateEnter).toEqualTypeOf<(state: "initial" | "final", action: (...args: any) => void) => KEventController>
    })
})